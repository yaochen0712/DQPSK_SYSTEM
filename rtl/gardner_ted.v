...
module gardner_ted
#(
    parameter OVSAMP_COEF = 4,
    parameter [19:0] DECISION_THRESHOLD = 20'd10
)
(
    ...
    localparam [15:0] wn_init_val = (1<<16)/OVSAMP_COEF;
    //寄存strobe_flag的次数
    reg [7:0]   strobe_cnt          ;
    ...
    wire        samp_flag           ;
    wire [19:0] abs_interpolate_I   ;
    wire [19:0] abs_interpolate_Q   ;
    wire        decision_reliable   ;

    assign abs_interpolate_I = interpolate_I[19] ? (~interpolate_I + 20'd1) : interpolate_I;
    assign abs_interpolate_Q = interpolate_Q[19] ? (~interpolate_Q + 20'd1) : interpolate_Q;
    assign decision_reliable = (abs_interpolate_I >= DECISION_THRESHOLD) &&
                               (abs_interpolate_Q >= DECISION_THRESHOLD);
